...
func (l Link) String() string {
	...
	return fmt.Sprintf("<%s>; %s", l.URLRaw, strings.Join(p, "; "))
}
...
func Parse(raw string) Links {
	...
	for _, chunk := range strings.Split(raw, ",") {
		...
		link := Link{URLRaw: "", Rel: "", Params: make(map[string]string)}
		...
		for _, piece := range strings.Split(chunk, ";") {
			...
			if piece[0] == '<' && piece[len(piece)-1] == '>' {
				link.URLRaw = strings.Trim(piece, "<>")
				link.URL, _ = url.Parse(link.URLRaw)
				continue
			}
